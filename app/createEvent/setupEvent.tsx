...
export default function SetupEvent() {
  ...
  const uploadCoverImageToFirebase = async (uri: string): Promise<string> => {
    try {
      console.log('Uploading cover image...');
      const response = await fetch(uri);
      const blob = await response.blob();

      const fileName = `${eventId}_cover_${Date.now()}.jpg`;
      const storageRef = ref(storage, `eventCovers/${fileName}`);
  ...
  const handleContinue = async () => {
    ...
    try {
      ...
      let coverImageURL = '';
      try {
        coverImageURL = await uploadCoverImageToFirebase(coverImageUri);
      } catch (uploadError) {
