...
export default function EventMaker() {
  ...
  const fetchAllEvents = async () => {
    try {
      ...
      // Fetch all events where user is in joinedUsers array
      const allEventsSnapshot = await getDocs(eventsRef);
      const userJoinedEvents: Event[] = [];
      
      for (const docSnap of allEventsSnapshot.docs) {
        const eventData = docSnap.data();
        const joinedUsers = eventData.joinedUsers || [];
        
        // Check if current user is in joinedUsers and is NOT the creator
        if (joinedUsers.includes(userId) && eventData.userId !== userId) {
          // Fetch the host's username
          const hostUserName = await fetchUserName(eventData.userId);
          
          userJoinedEvents.push({
            id: docSnap.id,
            ...eventData,
      ...
      console.log('Fetched my events:', userCreatedEvents.length);
      console.log('Fetched joined events:', userJoinedEvents.length);
    } catch (error) {
  ...
  const renderMyEventCard = ({ item }: { item: Event }) => (
    <TouchableOpacity
  ...
  const renderJoinedEventCard = ({ item }: { item: Event }) => (
    <TouchableOpacity
      style={styles.eventCard}
      onPress={() => handleEventPress(item)}
      activeOpacity={0.8}
    ...
    return (
      <>
        ...
        {joinedEvents.length > 0 && (
          <>
            <Text style={styles.sectionTitle}>My Joined Events</Text>
            {joinedEvents.map((event) => (
              <View key={event.id}>
                {renderJoinedEventCard({ item: event })}
              </View>
            ))}
          </>
        )}
