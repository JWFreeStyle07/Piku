...
const GRID_PADDING = SCREEN_WIDTH * 0.048; // Side padding
const ITEM_MARGIN = 5; // Margin between items
const MIN_ITEM_SIZE = 110; // Minimum item size
const MAX_ITEM_SIZE = 150; // Maximum item size for larger screens

// Calculate number of columns based on screen width
const calculateColumns = () => {
  const availableWidth = SCREEN_WIDTH - (GRID_PADDING * 2);
  const possibleColumns = Math.floor(availableWidth / (MIN_ITEM_SIZE + ITEM_MARGIN * 2));
  return Math.max(3, possibleColumns); // Minimum 3 columns, but can be more on larger screens
};

const NUM_COLUMNS = calculateColumns();

// Calculate item size to fit perfectly within screen
const calculateItemSize = () => {
  const availableWidth = SCREEN_WIDTH - (GRID_PADDING * 2);
  const totalMargin = (NUM_COLUMNS - 1) * ITEM_MARGIN * 2;
  const itemSize = (availableWidth - totalMargin) / NUM_COLUMNS;
  return Math.min(itemSize, MAX_ITEM_SIZE); // Cap at maximum size
};

const ITEM_SIZE = calculateItemSize();
...
const styles = StyleSheet.create({
  ...
  uploadText: {
    ...
    fontStyle: 'normal',
    fontWeight: '600',
    fontSize: Math.max(10, ITEM_SIZE * 0.1),
    lineHeight: Math.max(15, ITEM_SIZE * 0.15),
    color: '#FFFFFC',
    textAlign: 'center',
    paddingHorizontal: 5,
  },
